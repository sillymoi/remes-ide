package hr.fer.rasip.remes.converters.ui.wizards;

import hr.fer.rasip.remes.converters.Remes2UppaalflatConverter;
...
public class NewUppaalflatFileFromRemesCreationWizard extends Wizard{
	
	private WizardNewFileCreationPage remesFileCreationPage;
	private WizardNewFileCreationPage uppaalflatFileCreationPage;
	private IArchModel myArchModel;
	
	public NewUppaalflatFileFromRemesCreationWizard(IArchModel archModel){
		assert archModel != null : "ArchModel must be specified"; //$NON-NLS-1$
		
		remesFileCreationPage = new WizardNewFileCreationPage("Remes + ProSave to Uppaal transformation Wizard", StructuredSelection.EMPTY);
		...
		uppaalflatFileCreationPage = new WizardNewFileCreationPage("Remes + ProSave to Uppaal transformation Wizard", StructuredSelection.EMPTY);
		uppaalflatFileCreationPage.setTitle("Uppaal File");
		uppaalflatFileCreationPage.setDescription("Create new Uppaal file based on Remes and ProCom model");
		
		uppaalflatFileCreationPage.setContainerFullPath(filePath);
		uppaalflatFileCreationPage.setFileName(getUniqueFileName(filePath, fileName, "uppaal")); //$NON-NLS-1$
	...
	public void addPages() {
		...
		addPage(uppaalflatFileCreationPage);
	}
	...
	public boolean performFinish() {
		...
		IFile uppaalliteFile = uppaalflatFileCreationPage.createNewFile();
		RemesModelManager.createCompositeRemesModel(remesFile, myArchModel);
		Remes2UppaalflatConverter.doConvertArchitecture(remesFile, uppaalliteFile, myArchModel);
		return true;
	}
}