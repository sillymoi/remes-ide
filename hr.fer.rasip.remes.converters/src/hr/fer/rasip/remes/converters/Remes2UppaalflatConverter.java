 ...
 *
 */
public class Remes2UppaalflatConverter {
	...
	private static final String uppaalURI = UppaalPackage.eNS_URI; //$NON-NLS-1$
	
	private static URL asmURL;
	private static URL libAsmURL;
	
	static {
		// ATL remes2pta transformation
		Bundle bundle = Platform.getBundle(Activator.PLUGIN_ID); //$NON-NLS-1$
		asmURL = bundle.getEntry("resources/lib/r2u_module.asm"); //$NON-NLS-1$
		libAsmURL = bundle.getEntry("resources/lib/r2u.asm"); //$NON-NLS-1$
	...
	public static void doConvertArchitecture(IFile remesFile, IFile uppaalFile, IArchModel archModel){
		...
		for (Object element : modelResource.getContents()) {
			if (element instanceof ComponentImpl) {
				...
				if(componentResource != null) {
					IREMESModel behModel = REMESModel.getForComponent(componentResource);	
					doConvertBehaviour(modelResource, behModel, uppaalFile, remesFile);
				}
			}
		}
	}
	...
	public static void doConvertBehaviour(Resource modelResource, IREMESModel behModel, IFile uppaalliteFile, IFile remesFile) {


		String uppaalURI = uppaalliteFile.getLocationURI().toString();
		String remesUri = remesFile.getLocationURI().toString();
		try {
			remes2pta(modelResource, remesUri.toString(), uppaalURI.toString());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private static void remes2pta(Resource procomModelResource, String remesFile, String outputFile) throws Exception {
		...
		// Launching
		launcher.addLibrary("LIB", libAsmURL.openStream());
