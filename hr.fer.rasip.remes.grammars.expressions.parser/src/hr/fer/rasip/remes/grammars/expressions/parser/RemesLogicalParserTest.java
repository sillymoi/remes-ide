...
public class RemesLogicalParserTest {
	/**
	 * @param args
	 */
	public static void main(String[] args) {
		try {
			...
			lex = new RemesLexer(new ANTLRStringStream(
					//"x[24]>2 ? true : x[11] < 3"));
					"x [ 2 + 2 != 14 ] > 2+2 * 5 / 2 and 3 > 2"));

			// Run lexer
			CommonTokenStream tokens = new CommonTokenStream(lex);
			// Run parser
			RemesLogicalParser g = new RemesLogicalParser(tokens);
			// Create AST
			CommonTree tree = (CommonTree) g.invariant_expression().getTree();
...
//					folder.create(IResource.NONE, true, null);
				...
				LogicalRoot root = getExpression(tree);
	...
	private static LogicalRoot getExpression(Tree tree) 
	{
