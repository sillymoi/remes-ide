...
// </ul>
//
public class RemessrcBracketPreferencePage extends org.eclipse.jface.preference.PreferencePage implements org.eclipse.ui.IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = hr.fer.rasip.remes.resource.remessrc.ui.RemessrcPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new hr.fer.rasip.remes.resource.remessrc.mopp.RemessrcMetaInformation().getSyntaxName();
	
	private hr.fer.rasip.remes.resource.remessrc.ui.RemessrcBracketSet bracketsTmp;
	
	//
	// Creates a preference page for bracket setting.
	//
	public RemessrcBracketPreferencePage() {
		super();
		
		hr.fer.rasip.remes.resource.remessrc.IRemessrcMetaInformation metaInformation = new hr.fer.rasip.remes.resource.remessrc.mopp.RemessrcMetaInformation();
	...
	public void init(org.eclipse.ui.IWorkbench workbench) {
		setPreferenceStore(hr.fer.rasip.remes.resource.remessrc.mopp.RemessrcPlugin.getDefault().getPreferenceStore());
		setDescription("Define the coloring of matching brackets.");
		
		bracketsTmp = new hr.fer.rasip.remes.resource.remessrc.ui.RemessrcBracketSet(null, null);
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + hr.fer.rasip.remes.resource.remessrc.ui.RemessrcPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(		hr.fer.rasip.remes.resource.remessrc.ui.RemessrcPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		hr.fer.rasip.remes.resource.remessrc.ui.RemessrcPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.setBrackets(getPreferenceStore().getString(language + hr.fer.rasip.remes.resource.remessrc.ui.RemessrcPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		...
		bracketsList.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int index : itemIndices) {
					hr.fer.rasip.remes.resource.remessrc.IRemessrcBracketPair bracketPair = bracketsTmp.getBracketPair(index);
		...
		enableClosingInside.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int idx : itemIndices) {
					hr.fer.rasip.remes.resource.remessrc.IRemessrcBracketPair bracketPair = bracketsTmp.getBracketPair(idx);
	...
	protected void performDefaults() {
		enableCheckbox.setSelection(getPreferenceStore().getDefaultBoolean(		hr.fer.rasip.remes.resource.remessrc.ui.RemessrcPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		...
		bracketSetTemp.put(language, getPreferenceStore().getDefaultString(		language + hr.fer.rasip.remes.resource.remessrc.ui.RemessrcPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		bracketsTmp.setBrackets(bracketSetTemp.get(language));
		bracketsList.setItems(bracketsTmp.getBracketArray());
		enableClosingInside.setSelection(false);
	}
	...
	private void updateActiveEditor() {
		...
		getPreferenceStore().setValue(hr.fer.rasip.remes.resource.remessrc.ui.RemessrcPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		getPreferenceStore().setValue(language + hr.fer.rasip.remes.resource.remessrc.ui.RemessrcPreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		...
		if (editor != null && editor instanceof hr.fer.rasip.remes.resource.remessrc.ui.RemessrcEditor) {
			((hr.fer.rasip.remes.resource.remessrc.ui.RemessrcEditor) editor).invalidateTextRepresentation();
		}
	}
