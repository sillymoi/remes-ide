...
public class RemessrcOccurrence {
	...
	private hr.fer.rasip.remes.resource.remessrc.ui.RemessrcTokenScanner tokenScanner;
	...
	private hr.fer.rasip.remes.resource.remessrc.IRemessrcTextResource textResource;
	...
	//
	public RemessrcOccurrence(hr.fer.rasip.remes.resource.remessrc.IRemessrcTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, hr.fer.rasip.remes.resource.remessrc.ui.RemessrcTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new hr.fer.rasip.remes.resource.remessrc.mopp.RemessrcMetaInformation().getTokenNames();
	...
	public void handleOccurrenceHighlighting(hr.fer.rasip.remes.resource.remessrc.ui.RemessrcBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addPosition(document, hr.fer.rasip.remes.resource.remessrc.ui.RemessrcPositionCategory.DEFINTION.toString());
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addPosition(document, hr.fer.rasip.remes.resource.remessrc.ui.RemessrcPositionCategory.PROXY.toString());
					}
				}
			}
			token = tokenScanner.nextToken();
