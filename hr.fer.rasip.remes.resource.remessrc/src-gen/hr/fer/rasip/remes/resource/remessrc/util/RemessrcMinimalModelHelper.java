...
// models.
//
public class RemessrcMinimalModelHelper {
	
	private final static hr.fer.rasip.remes.resource.remessrc.util.RemessrcEClassUtil eClassUtil = new hr.fer.rasip.remes.resource.remessrc.util.RemessrcEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + hr.fer.rasip.remes.resource.remessrc.util.RemessrcStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = hr.fer.rasip.remes.resource.remessrc.util.RemessrcListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if(attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + hr.fer.rasip.remes.resource.remessrc.util.RemessrcStringUtil.capitalize(attribute.getName());
					}
					java.lang.Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = hr.fer.rasip.remes.resource.remessrc.util.RemessrcListUtil.castListUnchecked(value);
