...
public class UppaalliteCreationWizard extends Wizard implements INewWizard {
	...
	public void addPages() {
		diagramModelFilePage = new UppaalliteCreationWizardPage(
				"DiagramModelFile", getSelection(), "uppaallite_template_diagram"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		domainModelFilePage = new UppaalliteCreationWizardPage(
				"DomainModelFile", getSelection(), "uppaallite_template") { //$NON-NLS-1$ //$NON-NLS-2$

			public void setVisible(boolean visible) {
				if (visible) {
					String fileName = diagramModelFilePage.getFileName();
					fileName = fileName.substring(0, fileName.length()
							- ".uppaallite_template_diagram".length()); //$NON-NLS-1$
					setFileName(UppaalliteDiagramEditorUtil.getUniqueFileName(
							getContainerFullPath(), fileName,
							"uppaallite_template")); //$NON-NLS-1$
				}
